...
public class TextSelectionManager: NSObject {
    ...
    public var selectedLineBackgroundColor: NSColor = NSColor.selectedTextBackgroundColor.withSystemEffect(.disabled).withAlphaComponent(0.15)
    ...
    private func shouldUpdateCursorView(for textSelection: TextSelection) -> Bool {
        guard let cursorOrigin = layoutManager?.rectForOffset(textSelection.range.location)?.origin,
              let lineHeight = layoutManager?.estimateLineHeight() else {
            return false
        }
    ...
    private func configureCursorView(_ cursorView: CursorView, for textSelection: TextSelection) {
        ...
        cursorView.frame.size.height = lineHeight * 0.87
        let verticalOffset = (lineHeight - cursorView.frame.size.height) / 2
