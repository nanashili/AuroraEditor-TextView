...
public class GutterView: NSView {
    ...
    @Invalidating(.display)
    var selectedLineColor: NSColor = NSColor.selectedTextBackgroundColor.withSystemEffect(.disabled).withAlphaComponent(0.15)
    ...
    // Used for line number clicking (eg: debugging)
    public var clickedLineNumbers: Set<Int> = []
    public var lineNumberBackgroundImage: NSImage?
    public var lineNumberPositions: [Int: CGPoint] = [:]

    // Used for Git Version Control
    var editedLineNumbers: Set<Int> = []
    var hoveredDiffRangeIndex: Int?

    // Stores the ranges of line numbers that are highlighted.
    // This should be updated based on the output from parsing a git diff.
    var highlightedLineRanges: [CountableClosedRange<Int>] = []
    ...
    private func drawLineNumbers(_ context: CGContext) {
        ...
        if lineNumberBackgroundImage == nil {
            lineNumberBackgroundImage = createBackgroundImage()
        }

        // Reset the line number bounds
        lineNumberPositions.removeAll()
        ...
        for linePosition in textView.layoutManager.visibleLines() {
            ...
            // Store the position for each line number
            lineNumberPositions[linePosition.index + 1] = CGPoint(x: xPos, y: yPos)
            ...
            if clickedLineNumbers.contains(lineNumber) {
                ...
                let backgroundRect = CGRect(x: textPosition.x, y: textPosition.y - font.ascender, width: maxWidth, height: font.lineHeight)
                let roundedBackgroundImage = createBackgroundImage()
                context.draw(roundedBackgroundImage.cgImage(forProposedRect: nil, context: nil, hints: nil)!, in: backgroundRect)
            }
    ...
    override public func draw(_ dirtyRect: NSRect) {
        ...
        drawChangedLinesIndicator(context, lineHeight: textView?.lineHeight ?? 0)
        CATransaction.commit()
    }

    override public func mouseDown(with event: NSEvent) {
        handleBreakpointMouseDown(with: event)
